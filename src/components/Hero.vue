<template>
  <section class="w-full py-6 px-10">
    <div class="flex items-center sm:flex-row flex-col rounded-md">
      <div
        class="w-1/2 flex flex-col gap-4 py-4 px-2 md:text-start text-center"
      >
        <div class="">
          <h1 class="text-xl md:text-5xl font-bold text-blue-600">
            GRAB upon 50% Off on
          </h1>
          <h1 class="text-xl md:text-5xl text-blue-600 font-bold">
            Selected Toko Buku.
          </h1>
        </div>
        <!-- <div class="px-2 md:mx-0 mx-auto">
          <button
            class="block md:py-4 md:px-6 py-2 px-4 rounded-md bg-blue-600 text-white"
          >
            Buy Now
          </button>
        </div> -->
      </div>
      <div class="w-1/2">
        <img
          class="rounded-lg"
          src="https://gits.id/wp-content/uploads/2021/02/Contoh-Aplikasi-E-commerce-dan-Website.jpg"
        />
      </div>
    </div>
  </section>
</template>

<style scoped></style>
